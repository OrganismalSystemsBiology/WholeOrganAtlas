	...
	if (i_retCode != AT_SUCCESS) {
		...
		if (iNumberDevices <= 0) {
			...
			if (i_retCode != AT_SUCCESS) {
				...
				if (i_retCode == AT_SUCCESS) {
					cout << "Pixel Encoding set to Mono16" << endl << endl;
				}

				int iret = AT_SetEnumeratedString(Hndl, L"TriggerMode", L"External Exposure");
				if (iret != AT_SUCCESS) {
					cout << "Error setting trigger mode to External Exposure, retcode=" << iret << endl;
				}
				else {
					cout << "Trigger mode set to External Exposure" << endl << endl;
				}
...
int getUserSettings(int _handle){
	...
	if (i_rate == 100) {
		...
		i_retCode = AT_SetEnumeratedString(_handle, L"PixelReadoutRate", L"560 MHz");
	}
...
int doLoopOfAcquisition(int _handle, int _i_count)
{
	...
	AT_Command(_handle, L"Acquisition Start");
